...
const Home = () => {
    ...
    useEffect(() => {
        axios.get('https://mern-task-server-db76.onrender.com/get')
            .then(result => setTodos(result.data))
            .catch(err => console.log(err));
    }, []);

    const edit = (id) => {
        axios.put(`https://mern-task-server-db76.onrender.com/edit/${id}`)
            .then(result => {
    ...
    const Update = (id, updatedTask) => {
        axios.put(`https://mern-task-server-db76.onrender.com/update/${id}`, { task: updatedTask })
            .then(result => {
                console.log(result.data);
                const updatedTodos = todos.map(todo => {
                    if (todo._id === id) {
    ...
    const Hdelete = (id) => {
        axios.delete(`https://mern-task-server-db76.onrender.com/delete/${id}`)
            .then(result => {
                console.log(result.data);
